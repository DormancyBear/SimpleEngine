...
bool ShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	bool result = InitializeShader(device, hwnd, L"./media/light.vs", L"./media/light.ps");
	if (!result)
	{
		return false;
	}
...
bool ShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// D3D 的驱动程序( HAL )将 Shader 文本编译为 GPU 可识别的指令代码( 翻译得到的代码存放在一个 buffer 中 )
	...
	// 利用上面的代码 buffer 创建实际的 shader object
	...
	// 指定传入 vertex shader 的顶点数据布局( the layout of the vertex data )
	...
	// 纹理三角形与屏幕三角形的大小通常并不一致
	// 当纹理三角形比屏幕三角形小时, 纹理三角形必须被放大( magnified )才能填充屏幕三角形
	// 当纹理三角形比屏幕三角形大时, 纹理三角形必须被缩小( minfied )才能贴到屏幕三角形上
	// 这个过程会导致畸变, 所以有了纹理过滤技术 => 如何进行采样( 使得屏幕三角形上最终呈现的颜色过渡平滑而不突兀 )?
	// 所以 SamplerState 其实是用于指定采样方式, 如最近点采样, 线性纹理过滤 ...
	...
	// 每个 Shader 都在显存中 allocate 一个 constant buffer 用于存储 global variables
