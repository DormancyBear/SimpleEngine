...
namespace SimpleEngine
{
	namespace UI
	{
			...
			BaseLayout(scale_weight)
		{
			...
			// 让 widget_b 始终靠在窗口右下角, 其大小一直跟着窗口等比缩放
			widget_b->SetCoord(Rect<int>(-254, -300, 0, 0), Point<float>(1.0f, 1.0f), Point<float>(1.0f, 1.0f));

			HorizontalLayoutGroup *widget_c = new HorizontalLayoutGroup(widget_a);
			//std::shared_ptr<Image> image_c = std::make_shared<Image>(L"./media/Textures/1.jpg");
			...
			widget_d->SetCoord(Rect<int>(-128, -238, 0, 0), Point<float>(1.0f, 1.0f), Point<float>(1.0f, 1.0f));
			...
			widget_e->SetCoord(Rect<int>(-72, -100, 0, 0), Point<float>(1.0f, 1.0f), Point<float>(1.0f, 1.0f));
		}
	}
}